...
export const components = sqliteTable(
  'components',
  {
    ...
    // Where you buy it. Renderer renders as an external link that opens in
    // the system browser via the main process's windowOpenHandler.
    link: text('link'),
    colour: text('colour'),
    size: text('size'),

    // How many individual units are in one pack from the supplier.
    // Purely a UX aid for the receive-stock form (and a reorder reminder);
    // stock is still tracked in `unit`, never in packs.
...
export const productVariants = sqliteTable(
  'product_variants',
  {
    ...
    // Desired on-hand stock level. NULL means "not tracked" — the dashboard's
    // stock-target panel only shows variants with a target set.
    ...
    // How long it takes to make one of these (in hours, fractional allowed).
    // Combined with settings.labour_hourly_rate_pence in the cost roll-up.
    labour_hours_per_unit: real('labour_hours_per_unit'),

    // Free-form construction instructions: cut lengths, hole positions,
    // candle pour temps, drop counts of dye, etc. Markdown-friendly text
    // but no formatting is enforced.
...
// ---------------------------------------------------------------------------
// App settings — single-row table (id fixed to 1) for global preferences.
// Currently just labour rate; future: default currency, theme, etc.
...
// ---------------------------------------------------------------------------
// Files live on disk under `userData/assets/<storage_path>`. The renderer
// reads them via the `craft-asset://` custom protocol registered in main.
...
export const variantAttachments = sqliteTable(
  'variant_attachments',
  {
    ...
    variant_id: integer('variant_id')
      .notNull()
      .references(() => productVariants.id),
    // Original filename as uploaded — used for display/download labels.
    filename: text('filename').notNull(),
    // Relative path under userData/assets/, e.g. "variants/42/abc-uuid.png"
